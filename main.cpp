...
void add_shadows(const std::vector<Triangle> &triangles,
                 std::vector<Triangle> &triangles_shadows, Vector4 light_source,
...
void perform_culling(std::vector<Triangle> &triangles) {
  for (auto it = triangles.begin(); it != triangles.end();) {
    ...
    if (num_invisible_vertices == 3 ||
        a.cross(b).z <
            0) { // If all are outside the area, or the backside is seen
...
static bool is_under_other_triangle(const Triangle &t1, const Triangle &t2) {
  ...
  for (Vector4 v : t1.vertices) {
    if (a.cross(v - A).z > 0 && b.cross(v - B).z > 0 &&
        c.cross(v - C).z >
            0) { // Vertex is inside triangle from camera perspective
      Vector4 N = a.cross(b);
  ...
  std::sort(triangles.begin(), triangles.end(),
            [](const Triangle &t1, const Triangle &t2) {
              if (is_under_other_triangle(t1, t2))
                ...
                return !is_under_other_triangle(t2, t1);
            });
}

void sfml_draw_triangles(sf::RenderTarget &target,
...
int main(int, char const **) {
  ...
  sf::RenderWindow window(sf::VideoMode(800, 800), EXECUTABLE_NAME,
  ...
  while (window.isOpen()) {
    ...
    add_shadows(transformed_triangles, triangles_shadows, light_source,
                shadow_color);
